...
void Pet::InitStatsForLevel(uint32 petlevel)
{
    ...
    // Zeroed, because the switch below has a default arm that only logs: a pet
    // type it does not handle used to fall through to the multipliers underneath
    // and read these uninitialised, giving the pet whatever was on the stack --
    // garbage stats, or a NaN that then spreads through every derived value.
    // Zero is wrong too, but it is wrong the same way every time, and the error
    // line that accompanies it says so.
    float health = 0.0f;
    float mana   = 0.0f;
    float armor  = 0.0f;
    float minDmg = 0.0f;
