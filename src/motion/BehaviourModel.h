...
namespace Motion
{
    ...
    struct Sight
    {
        ...
        bool       runningState = false; ///< the shell's running state: a chase's or a fear's leg latched, or the creature's RUNNING gait (the old UNIT_STAT_RUNNING_STATE)
    ...
    /// A native's own latches (Effect::Latch): its presence (the chase or the follow is active)
    /// and its leg (a leg it laid runs). The kernel names its own; the shell maps the emitting
    /// native's kind to the channel (MotionMaster::WriteLatches), so no shell mask reaches here.
    enum LatchBit : uint8
    {
        LatchPresence = 0x01,
        LatchLeg      = 0x02,
        LatchBoth     = 0x03    ///< LatchPresence | LatchLeg: a suspension's or a finish's clear of both
    };
    ...
    /// creature, and the latch writes are every owner's.
    struct Effect
    {
        enum Kind : uint8
        {
            ...
            Latches,           ///< MotionMaster::WriteLatches(the emitting native's kind, latchSet, latchClear): the LatchBit bits set, then cleared, on the channel the kind names (a chase's or a follow's presence and leg, a fear's or a confuse's leg).
                               ///< Performed for EVERY owner (Effect::Owners: OwnerAny): a feared or confused player carries its leg exactly as the generators wrote its move bit; the
                               ///< shell's loop lifts the creature-only rule for this kind since P5-B family 4.
            WipeLatches,       ///< MotionMaster::WipeLatches(): the Home native's first-tick wipe of the unit's dynamic state (every latch, the published block, the shell's dynamic unit-state bits); every owner's, as the mask it replaces was
        ...
        uint8        latchSet;   ///< Latches: the LatchBit bits to set
        uint8        latchClear; ///< Latches: the LatchBit bits to clear
        ...
        Effect(Kind k, Motion::Kind w = Motion::Kind::Idle, uint32 i = 0) : kind(k), who(w), id(i), raw(0), event(PathEvent::NodeReached), flag(false), latchSet(0), latchClear(0), angle(0.0f), reason(FinishReason::Arrived) {}
        ...
        static Effect Latch(uint8 set, uint8 clear) { Effect e(Latches); e.latchSet = set; e.latchClear = clear; return e; }
        ...
        /// The owners of a kind. The latch writes are a player's as much as a creature's (a
        /// feared player carries its fear's leg exactly as the generators wrote the move bit); the
